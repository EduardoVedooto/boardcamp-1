...
export async function getRentals(req, res) {
    ...
    if (customerId) {
        filter = `WHERE customers.id=${sqlstring.escape(customerId)}`;
    }

    if (gameId) {
        filter = `WHERE games.id=${sqlstring.escape(gameId)}`;
    }

    try {
        const rentalsString = await db.query({
            text: `
                SELECT 
                    rentals.*, 
        ...
        const rentalsList = rentalsString.rows.map(row => {
            const [
...
export async function deleteIdRental(req, res) {
    const { id } = req.params;
